...
def getBatch(batch_size, train_data,step):
    if (step+1)*batch_size <= len(train_data):
        batch = train_data[step*batch_size:(step+1)*batch_size]
    else:
        batch = train_data[step*batch_size:len(train_data)]
    return batch
...
def to_index(train, word2index, intent2index):
    ...
    for sin, sout, intent in train:
        ...
        new_train.append([sin_ix, intent_ix])
    return new_train,len(new_train)